...
- **태스크바 위젯**: 5시간/7일 사용량을 원형 프로그레스 링 + 퍼센트 숫자로 실시간 표시
- **드래그 이동**: 위젯을 드래그하여 원하는 위치로 이동 (위치 자동 저장)
- **위젯 우클릭 메뉴**: 위치 고정/해제, 위치 초기화
...
- **시스템 트레이**: Claude 아이콘 + 우클릭 메뉴 (주간 사용량 토글, 위치 초기화, 시작 시 실행, 종료)
- **주간 사용량 토글**: 트레이 메뉴에서 주간 링 표시/숨기기 선택 가능
